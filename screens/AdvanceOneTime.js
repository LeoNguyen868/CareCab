...
const AdvanceOneTime = () => {
  const navigation = useNavigation();
  return (
    <Pressable style={styles.AdvanceOneTime}>
...
`}</Text>
          ...
          <Text style={styles.dchVA}>
            {" "}
            Dịch vụ hỗ trợ cá nhân cho người cần sự trợ giúp đặc biệt.
          </Text>
        </Text>
        <Text style={[styles.price, styles.inforTypo]}>
          <Text style={styles.aNTiTypo}>Giá dịch vụ:</Text>
          <Text style={styles.dchVA}> {`100.000VNĐ/Giờ từ 4 giờ trở lên áp dụng giá 95.000VNĐ/Giờ.
`}</Text> 
        </Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  infor: {
    top: 42 * height / 800,
    width: 330 * width / 360,
    height: 78 * height / 800,
  },
  price: {
    top: 11 * height / 800,
    width: 320 * width / 360,
    height: 40 * height / 800,
  },
  boxOut1: {
    top: 68 * height / 800,
    left: 6 * width / 360,
